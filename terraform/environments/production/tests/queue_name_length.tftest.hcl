...
# A deployment_name that fits keeps every derived queue name under the cap, so
# the gate passes and the plan proceeds.
run "queue_names_fit" {
  command = plan
...
# to 64 chars, three over the cap. The gate must fail the plan before any
# resource is touched.
