...
function getPendingDeadline(
  stub: DurableObjectStub
): Promise<PendingSandboxConnectDeadline | undefined> {
  return runInDurableObject(stub, (_instance: SessionDO, state) =>
    state.storage.get<PendingSandboxConnectDeadline>("pendingSandboxConnectDeadline")
  );
}

async function movePendingDeadline(stub: DurableObjectStub, deadlineAtMs: number): Promise<void> {
  await runInDurableObject(stub, async (_instance: SessionDO, state) => {
    const deadline = await state.storage.get<PendingSandboxConnectDeadline>(
      "pendingSandboxConnectDeadline"
    );
    if (!deadline) throw new Error("Production did not create a pending sandbox deadline");
    await state.storage.put("pendingSandboxConnectDeadline", { ...deadline, deadlineAtMs });
  });
}
...
describe("pending prompt connection deadline", () => {
  it("rearms after the connection alarm and fails durably without late dispatch", async () => {
    ...
    const productionDeadline = await getPendingDeadline(stub);
    expect(productionDeadline).toEqual({ messageId, deadlineAtMs: expect.any(Number) });
    expect(productionDeadline!.deadlineAtMs).toBeGreaterThanOrEqual(
    ...
    await movePendingDeadline(stub, deadlineAtMs);
    await runInDurableObject(stub, async (_instance: SessionDO, state) => {
  ...
  it("gives the next prompt a fresh deadline after the oldest one fails", async () => {
    ...
    expect(await getPendingDeadline(stub)).toEqual({
    ...
    await movePendingDeadline(stub, firstDeadlineAtMs);
    await runInDurableObject(stub, async (_instance: SessionDO, state) => {
      await state.storage.setAlarm(Date.now() + UNRELATED_ALARM_DELAY_MS);
    });
