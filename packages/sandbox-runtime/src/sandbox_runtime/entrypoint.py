...
class SandboxSupervisor:
    ...
    def _write_workspace_manifest(self) -> None:
        """Write the generated /workspace/AGENTS.md.

        Regenerated on every boot (restores included). This is the workspace-root
        file OpenCode reads first; every repo (single- or multi-) is checked out one
        level down at ./<repo>/, so writing here never clobbers a repository's own
        ./<repo>/AGENTS.md. Carries the repo layout plus a pointer to the bundled
        engineering skills. Skipped only for ad-hoc sessions with no repository.
        """
        if not self.repositories:
            return
        ...
        if self.is_multi_repo:
            ...
            lines.append("This session spans multiple repositories, checked out side by side:")
            lines.append("")
            lines.append("| Path | Repository | Base branch |")
            lines.append("| --- | --- | --- |")
            for repo in self.repositories:
            ...
            lines.append(
                f"The repository `{repo.owner}/{repo.name}` is checked out at `./{repo.name}/`."
            )
            lines.append("")
            if (repo.path / "AGENTS.md").exists():
                lines.append(
                    f"Repository-specific instructions are NOT loaded automatically — read "
                    f"`./{repo.name}/AGENTS.md` before working in it."
                )
                lines.append("")
    ...
    def _resolve_mcp_servers(self) -> list[dict]:
        ...
        for server in servers:
            ...
            if isinstance(command, list) and command and command[0] == "npx":
                package_index = next(
                    (
                        index + 1
                ...
                if package_index is not None and command[package_index] == "@playwright/mcp":
                    command = [*command]
                    command[package_index] = f"@playwright/mcp@{playwright_version}"
