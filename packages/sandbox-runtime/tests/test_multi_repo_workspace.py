...
class TestWorkspaceManifest:
    def test_writes_manifest_with_members_and_working_branch(self, tmp_path):
        ...
        # the engineering-skills nudge rides on every workspace manifest
        assert "Engineering skills available" in manifest
        assert "/matt-wayfinder" in manifest
    ...
    def test_single_repo_writes_manifest_with_skills_nudge(self, tmp_path):
        config = json.dumps({"session_id": "s", "branch": "main"})
        sup = _make_supervisor(tmp_path, session_config=config)
        (tmp_path / "frontend").mkdir()
    ...
    def test_single_repo_without_agents_md_omits_pointer_keeps_nudge(self, tmp_path):
        config = json.dumps({"session_id": "s", "branch": "main"})
        sup = _make_supervisor(tmp_path, session_config=config)
