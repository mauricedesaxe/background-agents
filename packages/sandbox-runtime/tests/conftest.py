...
def isolate_runtime_file_paths(tmp_path, monkeypatch):
    ...
    agent_session_id_path = str(tmp_path / "agent-session-id")
    legacy_session_id_path = str(tmp_path / "opencode-session-id")
    boot_completed_path = str(tmp_path / "oi-boot-completed")
    ...
    monkeypatch.setattr(
        "sandbox_runtime.runtime_config.AGENT_SESSION_ID_FILE_PATH", agent_session_id_path
    )
    monkeypatch.setattr(
        "sandbox_runtime.runtime_config.LEGACY_OPENCODE_SESSION_ID_FILE_PATH",
        legacy_session_id_path,
    )
    monkeypatch.setattr(
        "sandbox_runtime.runtime_config.BOOT_COMPLETED_FILE_PATH", boot_completed_path
    )
    monkeypatch.setattr(
        "sandbox_runtime.repository_boot.BOOT_COMPLETED_FILE_PATH", boot_completed_path
    )
    monkeypatch.setattr("sandbox_runtime.bridge.AGENT_SESSION_ID_FILE_PATH", agent_session_id_path)
    monkeypatch.setattr(
        "sandbox_runtime.bridge.LEGACY_OPENCODE_SESSION_ID_FILE_PATH", legacy_session_id_path
    )
