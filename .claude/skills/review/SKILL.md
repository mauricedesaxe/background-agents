...
- `git-hygiene-reviewer` — the shape of the *history and PR meta*, not the code:
  ...
  the stack (root `CLAUDE.md` "Version control: jj", PHILOSOPHY §28, the
...
- `yagni-reviewer` — speculative generality: the abstraction for one caller, the
  ...
  `code-reviewer`'s §14 bullet is expected; both run.
...
**App-specific reviewers** live at root `.claude/agents/`, namespaced and scoped by their
`description` to one app of the monorepo (they're kept at root, not in a nested
`apps/<app>/.claude/agents/`, because nested agent dirs don't register from a root-launched
session). Match them to the diff's app by changed path — see root `CLAUDE.md` → "App-specific
plugins" for the mapping. Current ones:

- `apps/walkup/**` diff → also spawn `geo-scoring-reviewer` (walkability domain), alongside
  the universal reviewers.
- `apps/lemonpi/**` diff → spawn `lemonpi-code-reviewer` **in place of** the
  generic `code-reviewer` (it's the stack-tuned version).

Don't run an app reviewer for a diff that doesn't touch its app.
...
`yagni-reviewer`, plus the app-specific reviewer(s) matching the diff's app (see the path
mapping above) — `geo-scoring-reviewer` for a walkup diff, and `lemonpi-code-reviewer`
instead of `code-reviewer` for a lemonpi diff. (`yagni-reviewer` runs alongside the
app-specific reviewers too — it's app-agnostic.)
...
the stack range (`trunk()..@`), and the PR number — or "no PR yet" if Step 1's
`gh pr view` found none. It runs its own read-only `jj log` / `jj bookmark list` /
`gh pr view` from there; tell it **not** to re-run `jj git fetch` (reuse this run's
already-resolved `trunk()` so it judges the same snapshot as the other reviewers).
