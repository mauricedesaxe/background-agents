...
- **§1** — the meta-rule. Every added moving part (a replica, a queue, a cache layer, a second
  service, an abstraction) must clear the earn-its-keep bar: a **named, currently-felt,
  specific problem** the simpler option doesn't solve. "Hypothetical, future, or aesthetic
  ('cleaner') reasons don't qualify."
- **§13** — the same rule for build-vs-buy: don't hand-roll a generic layer over a solved/paid
  thing for flexibility nobody needs.
- **§14** — the code-level form, verbatim: "Don't write the abstraction for one caller, the
  config knob nobody asked for, the generic over a single type, or the extension point for a
  future that hasn't filed an issue. Speculative generality is a cost paid now against a
  benefit that usually never arrives; when the second caller actually shows up, *that's* when
  you generalise — and you'll know the real shape by then."
- **§19** — over-engineering a personal tool (full RBAC scaffolding, multi-tenancy, an audit
  trail) on a project `CLAUDE.md` declares non-commercial.
...
- **The options object** once a function crosses ~3-4 params (§14) — that's the sanctioned shape,
  not an unasked-for knob.
...
When invoked on a **plan** (from `/work` Step 4) rather than a diff, apply the same lens to what
the plan proposes to *build*: flag planned abstractions, config knobs, extension points, generic
layers, speculative schema, and premature infrastructure that no current, felt need justifies —
and planned over-engineering of a non-commercial tool (§19). Plan time is the cheapest place to
delete a seam, because the cost of removing it is a sentence, not a diff.

`plan-reviewer` already flags architecture deviations (its §13 "Architecture deviations" bullet)
and scope drift — don't restate its findings. Your angle is narrower and complementary: not "is
Postgres the right store" but "is *any* of this proposed machinery needed **yet**, or is it built
for a caller/feature the plan itself admits is hypothetical." If the plan says "we'll add a
`Strategy` interface so we can swap implementations later," that's your finding regardless of
whether the one implementation is correct.
