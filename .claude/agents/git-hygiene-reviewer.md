...
- **Conventional-commit format.** Subject matches
  ...
  malformed subject locally — CI re-enforces it, but flag it here first). Flag a missing or
  wrong type prefix.
...
- **`Closes #N` points at the correct, existing issue.** Read `closingIssuesReferences` (or
  ...
  clearly should. **Collision trap:** in-code `#NNN` references inside `apps/walkup` are
  *legacy standalone-repo* numbers (a filter-repo artifact) and collide with live
  lazar-ecosystem issue numbers — don't validate a `Closes #N` against an issue just because
  the number matches; confirm the issue's subject fits the change.
